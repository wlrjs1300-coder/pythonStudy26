...
def save_members(): # 메모리상에 리스트를 파일로 저장
    ...
    """
    with open(FILE_NAME, "w", encoding="utf-8") as f: # with는 자동으로 close 진행
        #       상수     덮어쓰기        한글처리용   파일객체(파일에 들어있는 변수)
        #                a = 추가
        # 메모리에 있는 리스트를 |로 연결하여 한줄로 저장
        for member in members: # members : 메모리에 있는 2차원 배열
...
def member_admin():
    ...
    if members[idx][3] != "admin":
        ...
        i = 0
        for i in range(len(members)):
            print(f"{i+1}. 아이디 : {members[i][0]} 비밀번호 : {members[i][1]} 이름 : {members[i][2]} 권한 : {members[i][3]} 활동상태 : {members[i][4]}\n")
        manage_member = input("관리 회원 아이디 : ").lower()
        for iidx, uuser in enumerate(members):
            if uuser[0] == manage_member:
                print(members[iidx])
        ...
        admin_select = input(">>>")
        if admin_select == "1":
            print("회원 활성화/비활성화 메뉴입니다")
            for member in members:
                if member[0] == manage_member:
                    if member[4] == True:
                        active_select = input("비활성화 하시겠습니까? 맞으면 Y : ").lower()
                        if active_select == "y":
                            member[4] = False
                            print("비활성화 되었습니다")
                        else:
                        ...
                        active_select2 = input("활성화 하시겠습니까? 맞으면 Y : ").lower()
                        if active_select2 == "y":
                            member[4] = True
                            print("활성화 되었습니다")
                        else:
            ...
            print("회원 권한 수정 메뉴입니다")
            for member in members:
                if member[0] == manage_member:
                    if member[3] == "manager":
                        role_select = input("일반 유저로 변경하시겠습니까? 맞으면 Y : ").lower()
                        member[3] = "user"
                        print("일반 유저로 변경 되었습니다")
                    elif member[3] == "user":
                        role_select2 = input("매니저로 변경하시겠습니까? 맞으면 Y : ").lower()
                        member[3] = "manager"
                        print("매니저로 변경 되었습니다")
                    elif member[3] == "admin":
                        print("변경 권한이 없습니다")
            ...
            blacklist_select = input("블랙리스트에 추가하시겠습니까? 맞으면 Y : ").lower()
            if blacklist_select == "y":
                for member in members:
                    if member[0] == manage_member:
                        black.append(members[iidx])
                        members.pop(iidx)
                        print("블랙리스트에 추가되었습니다")
                    else:
                        return
            else:
                return
...
def member_logout():
    ...
    if session is None:
        ...
        logout_select = input("로그아웃 하시려면 Y : ").lower()
        if logout_select == "y":
            session = None
            print("로그아웃 되었습니다")
        else:
...
def member_modify():
    ...
    if session is None:
        ...
        modify_select = input(">>>")
        if modify_select == "1":
            members[idx][2] = input("변경할 이름 : ")
            print(f"{members[idx][2]}로 이름이 변경되었습니다")
        elif modify_select == "2":
            members[idx][1] = input("변경할 비밀번호 : ")
            print(f"{members[idx][1]}로 비밀번호가 변경되었습니다")
...
def member_delete():
    ...
    if session is None:
        ...
        del_select = input("정말 탈퇴하시겠습니까? 맞으면 Y : ").lower()
        if del_select == "y":
            members.pop(idx)
            session = None
            print("탈퇴 되었습니다")
