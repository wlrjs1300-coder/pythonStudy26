...
def member_login():
    ...
    uid = input("ID : ")
    pw = input("Password : ") # 키보드로 아이디와 비밀번호 입력받아 변수에 넣음

    # 회원 목록에서 아이디를 찾아야함
    for idx, member in enumerate(members):
        # enumerate(): for문 안에 있으면 인덱스를 찾아옴
        # print("idx : ", idx) # idx = 1차원 주소
        # print("member : ", member) # member = 2차원 리스트
        # idx: 0
        # member: ['kkw', '1234', '김기원', 'admin', True]
        # idx: 1
        # member: ['lhj', '1234', '임효정', 'manager', True]
        # idx: 2
        # member: ['ljj', '1234', '이재정', 'user', True]
        # idx: 3
        # member: ['ljk', '1234', '이지건', 'user', True]
        if member[0] == uid: # 키보드로 입력한 uid와 리스트[00에 있는 값이 같으면
            if member[4] == False: # active 상태 확인 (False 이면)
                print("로그인할 수 없는 계정입니다. (블랙리스트/비활성화)")
                return # 로그인 중지
            # 비밀번호 확인
            if member[1] == pw: # 키보드로 입력한 pw와 리스트[1]에 있는 값이 같으면
                session = idx # 전역변수에 로그인한 주소 넣음
                ...
                print("비밀번호가 일치하지 않습니다. 초기 메뉴로 돌아갑니다")
                return
        # else: # 예외 발생 가능성 있음, for로 찾으면서 없으면 출력이 될 수 있음
        #     print("아이디를 찾을 수 없습니다. 초기 메뉴로 돌아갑니다")
        #     return
...
def member_admin():
    ...
    for i in range(len(members)):
        print(f"{i+1}. ID: {members[i][0]} PW: {members[i][1]} Name: {members[i][2]} Role: {members[i][3]} Active: {members[i][4]}\n")
    manage_user = input("아이디 지정 : ")
    for idx, member in enumerate(members):
        if member[0] == manage_user:
            member_admin_menu()
            admin_select = input(">>>")
            if admin_select == "1":
                print("회원 활성화/비활성화 메뉴입니다")
                if member[4] == True:
                    if input("비활성화 하시려면 Y : ").lower() == "y":
                        member[4] == False
                        print(f"{member[2]}님이 비활성화 되었습니다")
                    else:
                        return
                else:
                    if input("활성화 하시려면 Y : ").lower() == "y":
                        member[4] == True
                        print(f"{member[2]}님이 활성화 되었습니다")
            elif admin_select == "2":
...
def member_logout():
    ...
    if session is None:
        ...
        if input("로그아웃 하시려면 Y를 눌러주세요 : ").lower() == "y":
            session = None
            print("로그아웃 되었습니다")
# 로그아웃 함수를 종료
...
def member_modify():
    ...
    if session is None:
        ...
        member_modify_menu()
        if input(">>>") == "1":
            members[session][2] = input("변경하실 이름 : ")
            print("이름 변경완료")
            return
        elif input(">>>") == "2":
            members[session][1] = input("변경하실 비밀번호 : ")
            print("비밀번호 변경완료")
            return
        else:
            print("다시 입력해주세요")
...
def member_delete():
    ...
    if session is None:
        ...
        if input("회원 탈퇴 하시겠으면 Y : ").lower() == "y":
            members.pop(session)
            session = None
        else:
            print("다시 입력하세요")
